...
const COUNTRY_CODES = { KE:"+254",FR:"+33",GB:"+44",US:"+1",AE:"+971",ZA:"+27",AU:"+61",JP:"+81",TH:"+66" };
function enrichHotel(h) {
  if (h.pricePerNight) return h;
  const base = h.stars >= 5 ? 350 : h.stars >= 4 ? 180 : 85;
  const seed = (h.hotelId||h.name||"x").split("").reduce((a,c)=>a+c.charCodeAt(0),0);
  const price = Math.round(base * (1 + ((seed%20)-10)/100));
  const cc = COUNTRY_CODES[h.country] || "+1";
  const num = 200000000 + (seed % 99999999);
  return { ...h, pricePerNight:price, phone:`${cc} ${num}`.replace(/(\d{3})(\d{4})(\d{4})/,"$1 $2 $3"),
...
function buildMockOffers(hotel, checkIn, checkOut, adults) {
  ...
  return {
    hotelId:hotel.hotelId, name:hotel.name, cityCode:hotel.address, rating:hotel.stars,
    phone:hotel.phone, email:hotel.email, website:hotel.website,
    description:`Welcome to ${hotel.name}. Enjoy world-class hospitality in the heart of ${hotel.address}.`,
    ...
    photos:[],
    offers:[
      { offerId:`${hotel.hotelId}-STD`, roomType:"Standard Room", bedType:"Double Bed", adults:Number(adults), boardType:"ROOM_ONLY",          checkIn, checkOut, currency:"USD", price:(base*nights*Number(adults)).toFixed(2) },
      { offerId:`${hotel.hotelId}-DLX`, roomType:"Deluxe Room",   bedType:"King Bed",   adults:Number(adults), boardType:"BREAKFAST_INCLUDED",  checkIn, checkOut, currency:"USD", price:(base*1.4*nights*Number(adults)).toFixed(2) },
      ...(hotel.stars>=4?[{ offerId:`${hotel.hotelId}-STE`, roomType:"Junior Suite", bedType:"King Bed", adults:Number(adults), boardType:"HALF_BOARD", checkIn, checkOut, currency:"USD", price:(base*2.2*nights*Number(adults)).toFixed(2) }]:[]),
    ],
  };
}

const API = "http://localhost:5000";
const AMENITY_OPTIONS = ["SWIMMING_POOL","SPA","FITNESS_CENTER","AIR_CONDITIONING","RESTAURANT","PARKING","PETS_ALLOWED","AIRPORT_SHUTTLE","WIFI","BEACH","CASINO","JACUZZI","SAUNA","MASSAGE","BAR or LOUNGE","MINIBAR","ROOM_SERVICE","KIDS_WELCOME"];
const AMENITY_ICONS = { SWIMMING_POOL:"🏊",SPA:"💆",FITNESS_CENTER:"🏋️",AIR_CONDITIONING:"❄️",RESTAURANT:"🍽️",PARKING:"🅿️",PETS_ALLOWED:"🐾",AIRPORT_SHUTTLE:"🚌",WIFI:"📶",BEACH:"🏖️",CASINO:"🎰",JACUZZI:"🛁",SAUNA:"🧖",MASSAGE:"💆","BAR or LOUNGE":"🍸",MINIBAR:"🍹",ROOM_SERVICE:"🛎️",KIDS_WELCOME:"👶",BUSINESS_CENTER:"💼",MEETING_ROOMS:"🤝" };
const HOTEL_PHOTOS = {
  5:["photo-1566073771259-6a8506099945","photo-1551882547-ff40c63fe5fa","photo-1520250497591-112f2f40a3f4","photo-1571896349842-33c89424de2d","photo-1563911302283-d2bc129e7570","photo-1606402179428-a57976d71fa4","photo-1582719508461-905c673771fd","photo-1618773928121-c32242e63f39"],
  4:["photo-1542314831-068cd1dbfeeb","photo-1584132967334-10e028bd69f7","photo-1455587734955-081b22074882","photo-1631049307264-da0ec9d70304","photo-1445019980597-93fa8acb246c","photo-1549294413-26f195200c16"],
  3:["photo-1522798514-97ceb8c4f1c8","photo-1506059612708-99d6d2a6d3b7","photo-1574643156929-51fa098b0394","photo-1416331108676-a22ccb276e35"],
};
const hotelPhoto = (nameOrId, stars=4) => {
  ...
  return `https://images.unsplash.com/${pool[hash%pool.length]}?auto=format&fit=crop&w=400&h=240&q=80`;
};
const renderStars = (n) => "⭐".repeat(Math.min(Number(n)||0,5));

function useToast() {
  const [toasts, setToasts] = useState([]);
  const add = (msg, type="info") => { const id=Date.now(); setToasts(p=>[...p,{id,msg,type}]); setTimeout(()=>setToasts(p=>p.filter(t=>t.id!==id)),3500); };
    ...
    <div className="fixed bottom-6 right-6 z-[999] flex flex-col gap-2 pointer-events-none">
      {toasts.map(t=>(
        <div key={t.id} style={{animation:"slideIn .25s ease"}}
          className={`px-5 py-3 rounded-2xl shadow-xl text-sm font-semibold text-white ${t.type==="success"?"bg-emerald-500/90":t.type==="error"?"bg-red-500/90":"bg-gray-700/90"}`}>
          {t.msg}
        </div>
      ))}
    </div>
...
export default function HotelPage() {
  // ── Auth: allow admin AND staff ──────────────────────────────────────────────
  const { user } = useAuth();

  const resolveRole = (u) =>
    typeof u?.role === "object" ? u?.role?.name?.toLowerCase?.() : u?.role?.toLowerCase?.() ?? "";

  const userRole       = resolveRole(user);
  const isStaffOrAdmin = ["admin", "staff", "finance"].includes(userRole);  // can use the page
  const isAdmin        = userRole === "admin";                              // extra privileges (delete, add hotel)
  ...
  const activeFilterCount = selectedAmenities.length+selectedRatings.length+(hotelSource!=="ALL"?1:0)+(radius!==5?1:0);
  const resetFilters = () => { setSelectedAmenities([]); setSelectedRatings([]); setHotelSource("ALL"); setRadius(5); setRadiusUnit("KM"); };
  ...
  const filteredLocal = hotels.filter(h=>h.name?.toLowerCase().includes(localSearch.toLowerCase()));

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`${API}/api/hotels`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(form)});
      if (!res.ok){toast.error("Failed to add hotel");return;}
      toast.success("Hotel added ✅"); setModalOpen(false);
  ...
  const searchCity = async () => {
    ...
    setCityLoading(true); setCities([]); setSelectedCity(null); setSearchResults([]); setSelectedHotel(null); setOffers(null);
    try {
      let data=null;
      try {
      ...
      if(!data){
        ...
        data=MOCK_CITIES.filter(c=>c.name.toLowerCase().includes(kw)||c.country.toLowerCase().includes(kw)||c.cityCode.toLowerCase()===kw);
      }
      if(!data.length)toast.error("No cities found — try Nairobi, Dubai, Paris…");
      else setCities(data);
  ...
  const searchHotels = async (city) => {
    setSelectedCity(city); setCities([]); setSearchResults([]); setSelectedHotel(null); setOffers(null);
  ...
  // Block non-staff/admin entirely
  if (!isStaffOrAdmin) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-gray-400">
        <p className="text-5xl mb-4">🔒</p>
        <p className="font-semibold text-lg text-gray-600">Access Restricted</p>
        <p className="text-sm mt-1">You need staff or admin access to view hotels.</p>
      </div>
    );
  }
    ...
    <div style={{fontFamily:"'Poppins',sans-serif"}} className="space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">
            Search worldwide or manage partner hotels
            {/* Role indicator */}
            <span className={`ml-2 text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${
              isAdmin ? "bg-red-100 text-red-600" : "bg-blue-100 text-blue-600"
            }`}>{userRole}</span>
          </p>
        </div>
              ...
              {filteredLocal.map((h,i)=>(
                <div key={h._id||i} style={{animationDelay:`${i*60}ms`}}
                  ...
                  <div className="p-4">
                    ...
                    {/* Edit: all staff+admin can see; Delete: admin only */}
                    <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100">
                      <button onClick={()=>toast.info("Edit coming soon")}
                        className="flex-1 text-xs text-yellow-600 border border-yellow-200 rounded-lg py-1.5 font-semibold hover:bg-yellow-50 transition">
                        Edit
                      </button>
                      {isAdmin && (
                        <button onClick={()=>handleDelete(h._id)}
                          className="flex-1 text-xs text-red-500 border border-red-100 rounded-lg py-1.5 font-semibold hover:bg-red-50 transition">
                          Delete
                        </button>
                      )}
                    </div>
      ...
      {tab==="search" && (
        <div className="space-y-6">
          <div className="bg-white rounded-2xl shadow-md p-5 space-y-4">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <input type="text" placeholder="City e.g. Nairobi" value={cityInput}
                onChange={e=>setCityInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchCity()}
            ...
            <div className="flex items-center gap-3">
              <button onClick={()=>setShowFilters(v=>!v)}
                ...
                Advanced Filters
                {activeFilterCount>0&&<span className="bg-yellow-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{activeFilterCount}</span>}
              </button>
              {activeFilterCount>0&&<button onClick={resetFilters} className="text-xs text-red-400 hover:text-red-600 font-medium">Reset</button>}
            </div>
              ...
              <div className="border border-gray-100 rounded-2xl p-4 space-y-5 bg-gray-50">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      <input type="range" min={1} max={50} value={radius} onChange={e=>setRadius(Number(e.target.value))} className="flex-1 accent-yellow-500" />
                ...
                <div>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {[{value:"ALL",label:"All Sources",desc:"GDS + Aggregators"},{value:"DIRECTCHAIN",label:"Direct / GDS",desc:"Chains & distribution"},{value:"BEDBANK",label:"Bed Banks",desc:"Aggregators only"}].map(s=>(
          ...
          {hotelLoading && <div className="text-center text-gray-400 py-10 animate-pulse">🔍 Searching hotels in {selectedCity?.name}…</div>}
          ...
          {!hotelLoading&&searchResults.length>0&&!selectedHotel&&activeFilterCount>0&&(
            <div className="flex flex-wrap gap-2 items-center">
              ...
              {selectedRatings.map(r=><span key={r} className="bg-yellow-100 text-yellow-700 text-xs font-semibold px-3 py-1 rounded-full">{"⭐".repeat(Number(r))}</span>)}
              {selectedAmenities.map(a=><span key={a} className="bg-teal-50 text-teal-700 text-xs font-semibold px-3 py-1 rounded-full">{AMENITY_ICONS[a]||"✓"} {a.replace(/_/g," ")}</span>)}
          ...
          {!hotelLoading&&searchResults.length>0&&!selectedHotel&&(
            <div>
              <p className="text-sm text-gray-500 mb-3">
                ...
                {nights>0?` · ${nights} night${nights!==1?"s":""} · click for prices`:" · add dates, then click a hotel"}
              </p>
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {searchResults.map((h,i)=>(
                  <div key={h.hotelId} onClick={()=>getOffers(h)} style={{animationDelay:`${i*55}ms`}}
                    className="card-appear bg-white rounded-2xl shadow-md overflow-hidden hover:shadow-xl transition cursor-pointer group">
                    <div className="relative">
                      <img src={hotelPhoto(h.name,h.stars)} alt={h.name} className="h-40 w-full object-cover group-hover:scale-105 transition duration-300"
                      ...
                      {h.stars&&<span className="absolute top-2 right-2 bg-yellow-400 text-yellow-900 text-xs font-bold px-2 py-0.5 rounded-full shadow">{renderStars(h.stars)}</span>}
                    </div>
                    <div className="p-4">
                      ...
                      <div className="mt-2 flex items-baseline gap-1">
                        <span className="text-yellow-600 font-bold text-base">USD {h.pricePerNight?.toLocaleString()}</span>
                        <span className="text-gray-400 text-xs">/night</span>
                      </div>
                      <div className="mt-2 space-y-0.5">
                        {h.phone&&<p className="text-xs text-gray-500 flex items-center gap-1"><span>📞</span><a href={`tel:${h.phone}`} onClick={e=>e.stopPropagation()} className="hover:text-yellow-600 transition">{h.phone}</a></p>}
                        {h.email&&<p className="text-xs text-gray-500 flex items-center gap-1 truncate"><span>✉️</span><a href={`mailto:${h.email}`} onClick={e=>e.stopPropagation()} className="hover:text-yellow-600 transition truncate">{h.email}</a></p>}
                        {h.website&&<p className="text-xs text-gray-500 flex items-center gap-1"><span>🌐</span><a href={`https://${h.website}`} target="_blank" rel="noreferrer" onClick={e=>e.stopPropagation()} className="hover:text-yellow-600 transition truncate">{h.website}</a></p>}
                      </div>
          ...
          {offerLoading&&<div className="text-center text-gray-400 py-10 animate-pulse">💰 Loading prices for <strong>{selectedHotel?.name}</strong>…</div>}

          {offers&&!offerLoading&&(
            <div className="card-appear bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="px-6 pt-5">
                <button onClick={()=>{setSelectedHotel(null);setOffers(null);}} className="text-sm text-yellow-600 hover:text-yellow-800 font-semibold transition">← Back to results</button>
              </div>
              <div className="relative mt-3">
                <img src={offers.photos?.length>0?offers.photos[0]:hotelPhoto(offers.name,offers.rating)} alt={offers.name} className="w-full h-64 object-cover"
                ...
                {offers.rating&&<div className="absolute top-4 right-4 bg-yellow-400 text-teal-900 font-bold text-sm px-3 py-1 rounded-full shadow">{renderStars(offers.rating)}</div>}
              </div>
              <div className="p-6 space-y-5">
                ...
                {(offers.phone||offers.email||offers.website)&&(
                  <div className="bg-gray-50 rounded-xl p-4 grid sm:grid-cols-3 gap-3">
                    {offers.phone&&<div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Phone</p><a href={`tel:${offers.phone}`} className="text-sm font-semibold text-teal-700 hover:text-teal-900 transition">📞 {offers.phone}</a></div>}
                    {offers.email&&<div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Email</p><a href={`mailto:${offers.email}`} className="text-sm font-semibold text-teal-700 hover:text-teal-900 transition break-all">✉️ {offers.email}</a></div>}
                    {offers.website&&<div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-0.5">Website</p><a href={`https://${offers.website}`} target="_blank" rel="noreferrer" className="text-sm font-semibold text-teal-700 hover:text-teal-900 transition">🌐 {offers.website}</a></div>}
                  </div>
                )}
                {offers.amenities?.length>0&&(
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {offers.amenities.slice(0,12).map(a=><span key={a} className="bg-gray-100 text-gray-600 text-xs px-3 py-1 rounded-full">{AMENITY_ICONS[a]||""} {a.replace(/_/g," ")}</span>)}
                    </div>
                  </div>
                )}
                <div>
                  ...
                  <div className="space-y-3">
                    {offers.offers?.map(o=>(
                      <div key={o.offerId} className="border border-gray-100 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:border-yellow-300 hover:bg-yellow-50 transition">
        ...
        <div onClick={()=>setModalOpen(false)} className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div onClick={e=>e.stopPropagation()} className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {[{key:"name",label:"Hotel Name *",type:"text"},{key:"location",label:"Location",type:"text"},{key:"price",label:"Price/night ($)",type:"number"},{key:"rooms",label:"Total Rooms",type:"number"}].map(f=>(
                <input key={f.key} type={f.type} placeholder={f.label} value={form[f.key]} required={f.key==="name"}
              ...
              <button type="submit" className="w-full bg-yellow-600 hover:bg-yellow-700 text-white py-2.5 rounded-xl font-semibold text-sm transition">Add Hotel</button>
            </form>
          </div>
        </div>
      )}
